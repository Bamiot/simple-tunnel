...
import { PROTOCOL_VERSION, FrameType, isValidSubdomain } from '@simple-tunnel/protocol';
...
type TunnelInfo = {
  ...
  createdAt: number;
  nextStreamId: number;
  streams: Map<number, { reply: any; timeout: NodeJS.Timeout; headersSent: boolean }>;
};
    ...
    try {
      ...
      switch (msg.t) {
        ...
        case FrameType.REGISTER_TUNNEL: {
          const requested = msg.subdomain && isValidSubdomain(msg.subdomain) ? msg.subdomain : undefined;
          const sub = requested && !tunnels.has(requested)
            ? requested
            : generateRandomSubdomain();
          if (tunnels.has(sub)) {
            // extremely unlikely if random, ask client to retry with another sub
            send(socket, { t: FrameType.ERROR, code: 'SUBDOMAIN_TAKEN', message: 'Please retry' });
            return;
          }
          assignedSubdomain = sub;
          tunnels.set(sub, { ws: socket, tunnelId, subdomain: sub, createdAt: Date.now(), nextStreamId: 1, streams: new Map() });
          send(socket, { t: FrameType.REGISTERED, subdomain: sub, tunnelId });
          req.log.info({ sub }, 'Tunnel registered');
          break;
        }
        case FrameType.RESP_START: {
          const info = assignedSubdomain ? tunnels.get(assignedSubdomain) : undefined;
          if (!info) break;
...
app.all('/*', async (req, reply) => {
  ...
  const timeout = setTimeout(() => {
    info.streams.delete(streamId);
    try { reply.code(504).send({ error: 'Upstream timeout' }); } catch {}
  }, 30000);
  ...
  if (req.raw.readable && !['GET', 'HEAD'].includes(req.method)) {
    for await (const chunk of req.raw as any as AsyncIterable<Buffer>) {
      send(info.ws, { t: FrameType.REQ_DATA, tunnelId: info.tunnelId, streamId, chunk } as any);
    }
  }
  // Signal end of request body
  send(info.ws, { t: FrameType.END, tunnelId: info.tunnelId, streamId, phase: 'req' } as any);
  return;
});
...
app
  ...
  .then(() => app.log.info(`Server listening on :${PORT}, domain base ${DOMAIN_BASE}`))
  .catch((err) => {
    app.log.error(err);
    process.exit(1);
  });
